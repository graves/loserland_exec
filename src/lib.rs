...
use winapi::um::winnt::{MEM_RESERVE, MEM_COMMIT, MEM_DECOMMIT,
    PIMAGE_NT_HEADERS, LPCSTR, IMAGE_SNAP_BY_ORDINAL, 
    PIMAGE_SECTION_HEADER, IMAGE_SCN_CNT_UNINITIALIZED_DATA,
    IMAGE_SCN_CNT_INITIALIZED_DATA,
    IMAGE_SCN_MEM_DISCARDABLE, IMAGE_OPTIONAL_HEADER,
    ...
    PAGE_EXECUTE_READWRITE, PAGE_NOCACHE, IMAGE_SCN_MEM_NOT_CACHED,
    IMAGE_DATA_DIRECTORY, IMAGE_DIRECTORY_ENTRY_TLS, PIMAGE_TLS_DIRECTORY,
    PVOID, DLL_PROCESS_ATTACH, IMAGE_DIRECTORY_ENTRY_IMPORT,
    PIMAGE_IMPORT_DESCRIPTOR, PIMAGE_IMPORT_BY_NAME, PIMAGE_EXPORT_DIRECTORY,
    IMAGE_DIRECTORY_ENTRY_EXPORT};
use winapi::um::libloaderapi::{LoadLibraryA};
use winapi::shared::minwindef::{HMODULE, DWORD, WORD, HINSTANCE, LPVOID, FARPROC, HIWORD};
use winapi::um::winbase::IsBadReadPtr;
use std::mem;


type DllEntryProc = unsafe extern "stdcall" fn(hinstDLL: HINSTANCE, fdwReason: DWORD, lpReserved: LPVOID);

#[link(name = "kernel32")]
#[link(name = "user32")]
extern "stdcall" {
    pub fn GetProcAddress(hModule: *const winapi::shared::minwindef::HINSTANCE__, lpProcName: *const i8) -> *mut usize;
}
...
pub fn get_proc(name: &str, dll_data: &Vec<u8>) -> FARPROC {
    ...
    let aligned_image_size = align_value_up(nt_headers.OptionalHeader.SizeOfImage, 
        sysinfo.dwPageSize);
   ...
   copy_sections(&result, &pe, &nt_headers, &dll_data);
    ...
    let location_delta;
    unsafe {
        location_delta = (*result.headers).OptionalHeader.ImageBase - nt_headers.OptionalHeader.ImageBase;
    }
...
fn copy_sections(result: &MemoryModule, pe: &PeFile, nt_headers: &IMAGE_NT_HEADERS, dll_data: &Vec<u8>) {
    for (count, s) in pe.section_headers().iter().enumerate() {
        let mut section = image_first_section(result.headers);
        unsafe { section = section.offset(count as isize); }

        if s.SizeOfRawData == 0 {
            let section_size = nt_headers.OptionalHeader.SectionAlignment;
            let dest;
            unsafe {
                dest = VirtualAlloc(result.code_base.offset(s.VirtualAddress as isize / std::mem::size_of::<*mut std::ffi::c_void>() as isize) as *mut winapi::ctypes::c_void, 
                    section_size as usize, MEM_COMMIT, PAGE_READWRITE);
            };
        ...
        unsafe {
            dest = VirtualAlloc(offset as *mut winapi::ctypes::c_void, s.SizeOfRawData as usize, MEM_COMMIT, PAGE_READWRITE) as *mut libc::c_void;
        }
        ...
        let addr;
        unsafe { addr = (*section).Misc.PhysicalAddress_mut(); }
...
fn build_import_table(result: &mut MemoryModule) {
    ...
    import_desc = (result.code_base as usize + directory.VirtualAddress as usize) as *mut winapi::um::winnt::IMAGE_IMPORT_DESCRIPTOR;

    let mut proc_addr;
    unsafe {
        while IsBadReadPtr(import_desc as *const winapi::ctypes::c_void, 0 as usize) == 0 && (*import_desc).Name != 0 {
            let handle: HMODULE;
            ...
            if orig_first_thunk > 0 {
                ...
                func_ref = (result.code_base as usize + (*import_desc).FirstThunk as usize) as *mut FARPROC;
            } else {
                thunk_ref = (result.code_base as usize + (*import_desc).FirstThunk as usize) as *mut u64;
                func_ref = (result.code_base as usize + (*import_desc).FirstThunk as usize) as *mut FARPROC;
            }
            ...
            while *thunk_ref != 0  {
                if IMAGE_SNAP_BY_ORDINAL(*thunk_ref) {
                    ...
                    let thunk_data: PIMAGE_IMPORT_BY_NAME = (result.code_base as usize + (*thunk_ref) as usize) as *mut winapi::um::winnt::IMAGE_IMPORT_BY_NAME;
...
fn finalize_sections(result: &MemoryModule) {
    ...
    let phys;
    unsafe { phys = (*section).Misc.PhysicalAddress(); }
    ...
    unsafe { section_data.characteristics = (*section).Characteristics; }
    ...
    for i in 1..number_of_sections {
        ...
        let this_phys;
        unsafe { this_phys = (*this_section).Misc.PhysicalAddress(); }

        let section_address = (*this_phys as usize | image_offset as usize) as *mut usize;
        let address_aligned = align_address_down(section_address as *mut libc::c_void, result.page_size);
        let section_size = real_section_size(&result, this_section);

        if section_data.address_aligned == address_aligned || section_data.address as usize + section_data.size as usize > address_aligned as usize {
            let characteristics;
            unsafe { characteristics = (*this_section).Characteristics; }

            if (characteristics & IMAGE_SCN_MEM_DISCARDABLE) == 0 || (section_data.characteristics & IMAGE_SCN_MEM_DISCARDABLE) == 0 {
                section_data.characteristics = (section_data.characteristics | characteristics) & !IMAGE_SCN_MEM_DISCARDABLE;
            } else {
                section_data.characteristics |= characteristics;
            }

            section_data.size = (section_address as usize + section_size as usize) - section_data.address as usize;
        }
        ...
        unsafe { section_data.characteristics = (*this_section).Characteristics; }
    }
...
fn execute_tls(result: &MemoryModule) {
    ...
    let tls: PIMAGE_TLS_DIRECTORY = (result.code_base as usize + directory.VirtualAddress as usize) as PIMAGE_TLS_DIRECTORY;
    let callback;
    unsafe { callback = (*tls).AddressOfCallBacks as *const (); }
    let callback: *const unsafe extern "system" fn(DllHandle: PVOID, Reason: DWORD, Reserved: PVOID) = unsafe { std::mem::transmute(callback) };
    unsafe { (*callback)(result.code_base as PVOID, DLL_PROCESS_ATTACH, 0 as PVOID); }
    let address_of_entrypoint;
    unsafe { address_of_entrypoint = (*result.headers).OptionalHeader.AddressOfEntryPoint; }
    if address_of_entrypoint != 0 {
         let dll_entry;
         unsafe { dll_entry = (result.code_base as usize + (*result.headers).OptionalHeader.AddressOfEntryPoint as usize) as *const (); }
         let dll_entry: DllEntryProc = unsafe { std::mem::transmute(dll_entry) };
         unsafe { dll_entry(result.code_base as *mut winapi::shared::minwindef::HINSTANCE__, DLL_PROCESS_ATTACH, 0 as *mut winapi::ctypes::c_void); }
    }
}
...
fn get_proc_address(result: &MemoryModule, name: &str) -> FARPROC {
    ...
    let exports: PIMAGE_EXPORT_DIRECTORY = (result.code_base as usize + directory.VirtualAddress as usize) as PIMAGE_EXPORT_DIRECTORY;
    if HIWORD(name.as_ptr() as DWORD) == 0 {
        println!("HIIWORD");
        // TODO
    } else {
        unsafe {
            let mut name_ref = (result.code_base as usize + (*exports).AddressOfNames as usize) as *const DWORD;
            let mut ordinal = (result.code_base as usize + (*exports).AddressOfNameOrdinals as usize) as *const WORD;
            for _ in 0..(*exports).NumberOfNames {
                    let namep = (result.code_base as usize + (*name_ref) as usize) as *const libc::c_char;
                    let exported_name = std::ffi::CStr::from_ptr(namep);
                    let idx = *ordinal;

                    if exported_name.to_str().unwrap() == name {
                        let offset = (result.code_base as usize + (*exports).AddressOfFunctions as usize + (idx as usize * 4)) as *const DWORD;
...
fn finalize_section(result: &MemoryModule, section: &SectionFinalizeData) -> bool {
    ...
    if section.characteristics & IMAGE_SCN_MEM_DISCARDABLE != 0 {
        ...
        unsafe {
            ...
            {
                VirtualFree(section.address as *mut winapi::ctypes::c_void, section.size, MEM_DECOMMIT);
            }
    ...
    unsafe {
        ...
        VirtualProtect(section.address as *mut winapi::ctypes::c_void, section.size, protect, &mut old_protect);
    }
...
fn image_first_section(nt_headers: PIMAGE_NT_HEADERS) -> PIMAGE_SECTION_HEADER {
    ...
    unsafe { optional_hdr_ptr = &(*nt_headers).OptionalHeader as *const _; }

    let section: PIMAGE_SECTION_HEADER;
    unsafe { section = optional_hdr_ptr.offset(1 as isize) as PIMAGE_SECTION_HEADER; }
